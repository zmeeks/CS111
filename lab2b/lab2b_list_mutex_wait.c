...
long long int total_thread_mutexlock_wait_time = 0;
SortedList_t a_list;
SortedListElement_t *element_array = NULL;

pthread_mutex_t mutex_list;  
int volatile spin_lock;		
...
void element_array_key_init(SortedListElement_t *array, int size){	
	...
	for(i=0; i<size; i++){
		...
		array[i].key = (const char *)random_string;
	}
}
...
void *wrapper_d(void *elem){
	...
	for(i=0; i< iterations-1; i++){	
		SortedList_insert(&a_list, elem_ptr);	
		elem_ptr += threads;	
		}
	SortedList_insert(&a_list, elem_ptr);	
	//length lookup
	length = SortedList_length(&a_list);
	if(length == -1){
	...
	//lookup and delete
	for(i=0; i< iterations-1; i++){	
		list_elem = SortedList_lookup(&a_list, elem_ptr->key);
		if(list_elem == NULL){
...
void *wrapper_m(void *elem){
	...
	struct timespec thread_finish;
	struct timespec mutex_start;
	struct timespec mutex_finish;
	struct timespec mutex_time;
	struct timespec thread_time;
	
	long long int mutex_total = 0;
	long long int thread_total;
	...
	for(i=0; i< iterations-1; i++){
		...
		clock_gettime(CLOCK_MONOTONIC, &mutex_finish); 	
		mutex_time = ts_subtract(mutex_finish, mutex_start);
		mutex_total += nano_time(mutex_time);
		}
	pthread_mutex_lock (&mutex_list);	
	clock_gettime(CLOCK_MONOTONIC, &mutex_start);
	SortedList_insert(&a_list, elem_ptr);
	...
	thread_time = ts_subtract(thread_finish, thread_start);
	thread_total = nano_time(thread_time);	
	total_diff = thread_total - mutex_total;
	
	total_thread_mutexlock_wait_time += total_diff;
...
int main(int argc, char** argv) {
	...
	switch(sync_flag) {
		case 'd':
			...
			for(i=0;i<threads;i++){	
				pthread_create(&thread_array[i], NULL, wrapper_d, &num[i]);	
				if(z != 0){
		...
		case 'm':
			...
			clock_gettime(CLOCK_MONOTONIC, &time_start); 
			for(i=0;i<threads;i++){	
				pthread_create(&thread_array[i], NULL, wrapper_m, &num[i]);	
				if(z != 0){
					fprintf(stderr, "ERROR: pthread create failed! \n");
					exit(1);
				}	
		...
		case 's':
			...
			clock_gettime(CLOCK_MONOTONIC, &time_start); 
			for(i=0;i<threads;i++){	
				pthread_create(&thread_array[i], NULL, wrapper_s, &num[i]);	
				if(z != 0){
					fprintf(stderr, "ERROR: pthread create failed! \n");
					exit(1);
				}	
    ...
    int len = SortedList_length(&a_list);    
    if(len != 0){
	...
	long long int avg_tmlwt = total_thread_mutexlock_wait_time/(long long int)threads;
	/* avg_tmlwt = average thread mutexlock wait time */
	
	printf("%s,%d,%d,%d,%d,%lld,%lld,%lld\n", test_name, threads, iterations, lists, ops, total_time, avg_op_time, avg_tmlwt);
